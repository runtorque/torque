"""Draft playbook candidate extraction from Loom history."""
...
def _stable_labels(labels: list[str]) -> list[str]:
    return sorted(label for label in (labels or [])
                  if label and not label.startswith("loom:"))
...
def _task_kind(task) -> str:
    if "loom:human" in (task.labels or []):
        return "ask"
    return "action"
...
def _stage_record(task, attempts: list[dict], agent_history: dict[str, dict]) -> dict:
    ...
    blocked = ("loom:blocked" in (task.labels or [])) or _has_message(task, "blocked")
    errored = ("loom:error" in (task.labels or [])) or _has_message(task, "error")
...
def build_playbook_draft(candidate: dict, action_mgr, template_mgr,
    ...
    return {
        ...
        "description": (
            "Generated from Loom history. Review before publication."
        ),
        "match": {
