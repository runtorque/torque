...
HTTP pooling uses the aiohttp dependency already used by Loom's server.
"""
...
IDEMPOTENCY_ARG = "_loom_idempotency_key"
IDEMPOTENCY_HEADER = "X-Loom-Idempotency-Key"
DEFAULT_MAX_RETRIES = 3
DEFAULT_ATTEMPTS = DEFAULT_MAX_RETRIES + 1
DEFAULT_BACKOFFS = (0.5, 1.5, 3.0)
DEFAULT_HTTP_TIMEOUT = 30

# Agent-local loom_* writes. Read tools and memory reads/lists are omitted.
_LOOM_WRITE_TOOLS = {
...
def ensure_mcp_payload_idempotency(payload: dict) -> tuple[dict, str, str]:
    ...
    argument understood by Loom's own server and stripped before tool dispatch.
    """
...
async def replay_failed_writes(db, sender: Callable[[dict], Awaitable[Any]], *, limit: int = 100) -> dict:
    ...
    the write reached Loom's command surface. Tool-level validation errors count
