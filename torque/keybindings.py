"""Global iTerm2 key bindings for Torque.
...
def _is_ours(binding):
    """Check if a binding was installed by Torque."""
...
def _find_cell_for_session(state, session_id=""):
    """Find the Torque cell for an invocation or active session."""
...
async def install(connection, specs=None):
    ...
    # Remove stale Torque bindings AND displaced originals
...
async def setup(connection, state, bridge, overrides=None, *,
    ...
    @iterm2.RPC
    async def torque_close_cell(session_id=iterm2.Reference("id")):
        cell = _find_cell_for_session(state, session_id)
        if cell:
            ...
            return
        log.info("Close-cell shortcut ignored — no Torque cell for invocation session %s",
                 _normalize_session_id(session_id) or state.active_session_id)

    @iterm2.RPC
    async def torque_add_agent(session_id=iterm2.Reference("id")):
        ...
        if group:
            ...
            log.info("Add-agent shortcut ignored — no Torque group available")

    @iterm2.RPC
    async def torque_add_engineer():
        await _send_action("add_engineer")

    @iterm2.RPC
    async def torque_add_architect(session_id=iterm2.Reference("id")):
        source_cell = _find_cell_for_session(state, session_id)
        group = source_cell.group if source_cell else None
        if not group:
            groups = list(state.groups.keys())
        ...
        if group:
            ...
            log.info("Add-architect shortcut ignored — no Torque group available")

    @iterm2.RPC
    async def torque_add_terminal(session_id=iterm2.Reference("id")):
        # Find the agent associated with the active session
        ...
        if current_agent:
            ...
            log.info("Add-terminal shortcut ignored — no Torque agent for invocation session %s",
                     _normalize_session_id(session_id) or state.active_session_id)

    # Register all RPCs
    await torque_focus_next.async_register(connection, timeout=10)
    await torque_focus_prev.async_register(connection, timeout=10)
    await torque_next_agent.async_register(connection, timeout=10)
    await torque_prev_agent.async_register(connection, timeout=10)
    await torque_close_cell.async_register(connection, timeout=10)
    await torque_add_agent.async_register(connection, timeout=10)
    await torque_add_engineer.async_register(connection, timeout=10)
    await torque_add_architect.async_register(connection, timeout=10)
    await torque_add_terminal.async_register(connection, timeout=10)
    log.info("Torque RPCs registered")
