...
def _call_digest_db_with_lock_retry(db, operation, *, attempts: int = 4):
    # TorqueDB owns transactional retry/health semantics for real SQLite writes.
...
- **Derived tasks** = Torque-created workflow handoffs, often dispatched
...
9. **Dispatch strategy** — Reuse context, but keep branch boundaries
   ...
   up front instead of scattering them across workers.  Torque actions and
...
class EngineerEventBuffer:
    ...
    def _format_digest(self, group: str, events: list[dict], board_summary: str,
        ...
        lines = [f"## Torque Digest ({len(events)} event"
