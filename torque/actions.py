...
# ---------------------------------------------------------------------------
# Minimal YAML parser (same as bin/torque — stdlib only, covers actions)
# ---------------------------------------------------------------------------
...
# Stub torque context for preview renders (no real agent/task).
# Templates referencing torque.* get safe defaults instead of StrictUndefined errors.
TORQUE_CONTEXT_STUB = {
...
class ActionManager:
    ...
    def validate_prompt(prompt: str) -> bool:
        ...
        return bool(
            ...
            or re.search(r'\{\{\s*torque\.task\.title\s*(\|[^}]*)?\}\}',
                         prompt))

    def render_prompt(self, name: str, variables: dict,
                      base_dir: str = "",
                      torque_context: dict | None = None) -> str | None:
        ...
        ``torque_context`` is injected as the ``torque`` namespace so
        templates can branch on agent state (e.g. ``torque.context.is_clean``).
        """
    ...
    def find_actions_dirs(base_dir: str = "") -> list[str]:
        ...
        .torque/actions/, then falls back to ~/.torque/actions/.
        ...
        for _ in range(20):
            ...
            if parent == d:
                break
            d = parent
        # Global: ~/.torque/actions/ (expand at call time for safety)
        g = os.path.expanduser("~/.torque/actions")
    ...
    def list_actions(self, base_dir: str = "") -> list[dict]:
        ...
        for tdir in self.find_actions_dirs(base_dir):
            is_global = (tdir == os.path.expanduser("~/.torque/actions"))
    ...
    def get_action_vars(self, raw_or_act) -> list[dict]:
        ...
        # Filter out 'torque' — it's a reserved namespace injected at render time
        discovered.discard("torque")
        result = []
    ...
    def render_action(self, act_or_name, variables: dict,
                      base_dir: str = "",
                      torque_context: dict | None = None) -> dict:
        ...
        for tr in transitions:
            if isinstance(tr, dict):
                ...
                if action_target:
                    ...
                    # Mandatory-review contract (TORQUE:256). ``required``
                    # marks a transition the worker MUST take before
                    # ``torque_done``; ``pre_approved`` marks a reviewer-issued
    ...
    def has_required_transition(self, action_name: str,
        ...
        ``requires_review=True`` and ``torque_done`` / ``torque_ready`` refuse
