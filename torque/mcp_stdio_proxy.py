"""Minimal MCP stdio proxy helpers for local Torque entrypoints."""
...
async def _proxy_request(payload: bytes, *, caller_id: str,
    ...
    headers = {
        ...
        "X-Torque-Cell-Id": str(caller_id or "").strip(),
    ...
    try:
        ...
        if queued and parsed.get("method") == "tools/call":
            return _tool_call_result(
                parsed.get("id"),
                (
                    "Torque write could not reach the daemon after retries; "
...
async def _serve_http_proxy_async(caller_id: str) -> int:
    """Proxy stdio-framed MCP requests to Torque's local HTTP endpoint."""
    reader = sys.stdin.buffer
    writer = sys.stdout.buffer
    mcp_session_id = (
        str(os.environ.get("TORQUE_MCP_SESSION_ID", "") or "").strip()
        or uuid.uuid4().hex
    )
    while True:
