...
class MatrixState:
    ...
    def _emit_initiative(self, initiative: dict | None) -> None:
        if not initiative:
            return
        self._emit("initiative_upsert", **dict(initiative))

    def resolve_initiative_id(self, identifier: str, *, group: str = "") -> str:
        ident = str(identifier or "").strip()
        if not ident or not self.db:
            return ""
        if self.db.load_initiative(ident):
            return ident
        group = str(group or "").strip()
        ident_lower = ident.lower()
        try:
            for item in self.db.list_initiatives(
    ...
    def list_initiatives(self, *, group: str = "",
        ...
        if not self.db:
            return []
        try:
            return self.db.list_initiatives(
                group=group,
                include_archived=include_archived,
            )
        except Exception:
    ...
    def load_initiative(self, initiative_id: str) -> dict | None:
        if not self.db:
            return None
        try:
            return self.db.load_initiative(initiative_id)
        except Exception:
    ...
    def initiative_links(self, initiative_id: str,
        ...
        if not self.db:
            return []
        try:
            return self.db.list_initiative_links(initiative_id, link_type)
        except Exception:
    ...
    def _initiative_linked_task_payload(self, task_ids: list[str], *,
        ...
        visible_task_ids = visible_task_ids if visible_task_ids is not None else set(self.board_tasks)
        items = []
    ...
    async def create_initiative_async(self, row_dict: dict) -> dict | None:
        if not self.db:
            return None
        try:
            saved = await self.db.create_initiative_async(row_dict)
    ...
    async def update_initiative_async(self, initiative_id: str,
        ...
        if not self.db:
            return None
        try:
            saved = await self.db.update_initiative_async(initiative_id, patch)
            self._emit_initiative(saved)
            return saved
        except Exception:
    ...
    async def archive_initiative_async(self, initiative_id: str, **kwargs
        ...
        if not self.db:
            return None
        try:
            saved = await self.db.archive_initiative_async(initiative_id, **kwargs)
            self._emit_initiative(saved)
            return saved
        except Exception:
    ...
    async def save_initiative_link_async(self, initiative_id: str,
        ...
        if not self.db:
            return None
        try:
            link = await self.db.save_initiative_link_async(
                initiative_id,
                link_type,
                target_id,
                **kwargs,
            )
            self._emit("initiative_link_upsert", **dict(link))
            return link
        except Exception:
    ...
    async def delete_initiative_link_async(self, initiative_id: str,
        ...
        if not self.db:
            return False
        try:
            removed = await self.db.delete_initiative_link_async(
                initiative_id,
                link_type,
                target_id,
            )
    ...
    def _emit_area(self, area: dict | None) -> None:
        if not area:
            return
        self._emit("area_upsert", **dict(area))

    def resolve_area_id(self, identifier: str, *, group: str = "") -> str:
        ident = str(identifier or "").strip()
        if not ident or not self.db:
            return ""
        if self.db.load_area(ident):
            return ident
        group = str(group or "").strip()
        ident_lower = ident.lower()
        try:
            for item in self.db.list_areas(
    ...
    def load_area(self, area_id: str) -> dict | None:
        if not self.db:
            return None
        try:
            return self.db.load_area(area_id)
        except Exception:
    ...
    def area_links(self, area_id: str, link_type: str = "") -> list[dict]:
        if not self.db:
            return []
        try:
            return self.db.list_area_links(area_id, link_type)
        except Exception:
    ...
    def area_notes(self, area_id: str, *, include_archived: bool = False,
        ...
        if not self.db:
            return []
        try:
            return self.db.list_area_notes(
                area_id,
                include_archived=include_archived,
                limit=limit,
            )
    ...
    def load_area_note(self, note_id) -> dict | None:
        if not self.db:
            return None
        try:
            return self.db.load_area_note(note_id)
        except Exception:
    ...
    async def create_area_async(self, row_dict: dict) -> dict | None:
        if not self.db:
            return None
        try:
            saved = await self.db.create_area_async(row_dict)
    ...
    async def update_area_async(self, area_id: str, patch: dict) -> dict | None:
        if not self.db:
            return None
        try:
            saved = await self.db.update_area_async(area_id, patch)
            self._emit_area(saved)
            return saved
        except Exception:
    ...
    async def archive_area_async(self, area_id: str, **kwargs) -> dict | None:
        if not self.db:
            return None
        try:
            saved = await self.db.archive_area_async(area_id, **kwargs)
            self._emit_area(saved)
            return saved
        except Exception:
    ...
    async def save_area_link_async(self, area_id: str, link_type: str,
        ...
        if not self.db:
            return None
        try:
            link = await self.db.save_area_link_async(
                area_id,
                link_type,
                target_id,
                **kwargs,
            )
            self._emit("area_link_upsert", **dict(link))
            return link
        except Exception:
    ...
    async def delete_area_link_async(self, area_id: str, link_type: str,
        ...
        if not self.db:
            return False
        try:
            removed = await self.db.delete_area_link_async(
                area_id,
                link_type,
                target_id,
                relation,
    ...
    async def create_area_note_async(self, area_id: str, row_dict: dict) -> dict | None:
        if not self.db:
            return None
        try:
            note = await self.db.create_area_note_async(area_id, row_dict)
            self._emit("area_note_upsert", **dict(note))
            return note
        except Exception:
    ...
    async def update_area_note_async(self, note_id, patch: dict) -> dict | None:
        if not self.db:
            return None
        try:
            note = await self.db.update_area_note_async(note_id, patch)
            self._emit("area_note_upsert", **dict(note))
            return note
        except Exception:
    ...
    async def archive_area_note_async(self, note_id, **kwargs) -> dict | None:
        if not self.db:
            return None
        try:
            note = await self.db.archive_area_note_async(note_id, **kwargs)
            self._emit("area_note_upsert", **dict(note))
            return note
        except Exception:
    ...
    def idea_brief_snapshot(self, *, group: str = "",
        ...
        try:
            return {
                ...
                for brief in self.db.list_idea_briefs(
                    group=group,
                    include_archived=include_archived,
                    limit=1000,
                )
    ...
    def _emit_idea_brief(self, brief: dict | None) -> None:
        if not brief:
            return
        self._emit("idea_brief_upsert", **dict(brief))
    ...
    def resolve_idea_brief_id(self, identifier: str, *,
        ...
        if not ident or not self.db:
            return ""
        brief = self.db.load_idea_brief(ident)
        if brief:
            return str(brief.get("id", "") or "")
        group = str(group or "").strip()
        ident_lower = ident.lower()
        try:
            for item in self.db.list_idea_briefs(
                    group=group,
                    include_archived=True,
                    limit=1000):
                if str(item.get("slug", "") or "").lower() == ident_lower:
    ...
    def list_idea_briefs(self, *, group: str = "",
        ...
        if not self.db:
            return []
        try:
            return self.db.list_idea_briefs(
                group=group,
                status=status,
                include_archived=include_archived,
    ...
    def load_idea_brief(self, brief_id: str) -> dict | None:
        if not self.db:
            return None
        try:
            return self.db.load_idea_brief(brief_id)
        except Exception:
    ...
    def normalize_idea_brief_thinking_links(self, links, *,
        ...
        for raw in links:
            ...
            if raw_type in {"scratchpad", "note", "scratchpad_note"}:
                note_id = self.resolve_scratchpad_note_id(ident, group=group)
                note = self.load_scratchpad_note(note_id)
                if (
                        not note
                        or note.get("deleted")
                ...
                map_id = self.resolve_mind_map_id(ident, group=group)
                ...
                node = self.load_mind_map_node(ident)
                ...
                link = self.load_mind_map_link(ident)
    ...
    async def create_idea_brief_async(self, row_dict: dict) -> dict | None:
        ...
        try:
            ...
            brief = await self.db.create_idea_brief_async(row)
    ...
    async def update_idea_brief_async(self, brief_id: str,
        ...
        try:
            ...
            brief = await self.db.update_idea_brief_async(brief_id, row)
            self._emit_idea_brief(brief)
            return brief
        except Exception:
    ...
    async def refine_idea_brief_async(self, brief_id: str,
        ...
        try:
            ...
            brief = await self.db.refine_idea_brief_async(brief_id, row)
            self._emit_idea_brief(brief)
            return brief
        except Exception:
    ...
    async def park_idea_brief_async(self, brief_id: str,
        ...
        if not self.db:
            return None
        try:
            brief = await self.db.park_idea_brief_async(brief_id, **kwargs)
            self._emit_idea_brief(brief)
            return brief
        except Exception:
    ...
    async def archive_idea_brief_async(self, brief_id: str,
        ...
        if not self.db:
            return None
        try:
            brief = await self.db.archive_idea_brief_async(brief_id, **kwargs)
            self._emit_idea_brief(brief)
            return brief
        except Exception:
    ...
    async def propose_idea_brief_async(self, brief_id: str,
        ...
        if not self.db:
            return None
        try:
            brief = await self.db.propose_idea_brief_async(brief_id, **kwargs)
            self._emit_idea_brief(brief)
            return brief
        except Exception:
