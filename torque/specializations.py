...
Parallel to ``torque/roles.py`` but operates at the engineer level:
...
Specialization YAML files live at ``~/.torque/specializations/*.yaml`` and
project ``.torque/specializations/*.yaml``. Read precedence: project
...
class SpecializationManager:
    ...
    GLOBAL_DIR = os.path.expanduser("~/.torque/specializations")

    @classmethod
    def _global_dir(cls) -> str:
        return os.path.expanduser("~/.torque/specializations")
    ...
    def _default_project_dir(cls, base_dir: str = "") -> str:
        d = os.path.expanduser(base_dir) if base_dir else os.getcwd()
        return os.path.join(d, ".torque", "specializations")
