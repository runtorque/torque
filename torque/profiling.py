"""Lightweight in-process profiling hooks for Torque perf harness runs.
...
def is_enabled() -> bool:
    ...
    if any(_env_truthy(name) for name in _PROFILE_ENV_NAMES):
        return True
    # TORQUE_PROFILE already names the runtime profile in normal operation.  To
...
class ProfileRecorder:
    ...
    def __init__(self, *, max_samples: int | None = None,
        ...
        self.max_samples = int(max_samples or os.environ.get(
            "TORQUE_PROFILE_MAX_SAMPLES", _DEFAULT_MAX_SAMPLES))
