...
class ITerm2BridgeCore:
    ...
    async def register_web_view_tool(self, *, display_name: str,
        ...
        """Register the Torque toolbelt webview in iTerm2."""
        try:
    ...
    async def create_session(self, cell: AgentCell, *,
        ...
        # Inject TORQUE_CELL_ID for all cells (used by hook correlation)
        env_vars = dict(env_vars) if env_vars else {}
        env_vars["TORQUE_CELL_ID"] = cell.id
            ...
            if hook_dir and hasattr(adapter, "install_skills"):
                if adapter.install_skills(hook_dir):
                    log.info("Installed skills for '%s' in %s",
                             cell.name, hook_dir)
            # Exclude Torque-injected files from git status
            if hook_dir:
                ensure_git_exclude(hook_dir)
            ...
            for gi, gname in enumerate(self.state.groups):
                ...
                for pos, aid in enumerate(self.state.groups[gname]):
                    ...
                    if cell and cell.session_id:
                        # Engineer gets position -1 so it sorts first
                        p = -1 if aid == engineer_id else pos
    ...
    async def send_text(self, session_id: str, text: str, *,
        ...
        actually submitting the prompt. By default Torque keeps a short
        ...
        if session:
            ...
            # Silent-drop guard (TORQUE:165 observability). Previously a
