...
def task_is_schedule_created(state, task: BoardTask | None) -> bool:
    """Return whether *task* was generated by a schedule, without mutation."""
    if not task:
        return False
    ...
    if not task_id:
        return False
    return any(
        str(getattr(schedule, "last_task_id", "") or "").strip() == task_id
        for schedule in getattr(state, "schedules", {}).values()
    )


def task_auto_tracks_for_board_sync(state, task: BoardTask | None, settings) -> bool:
    """Return the manager's pure local auto-create eligibility predicate."""
    return bool(
        group_auto_tracks_new_tasks(settings)
        and task_allows_auto_create_for_board_sync(task)
        and not task_is_schedule_created(state, task)
    )
...
def task_description_sync_limit_status(
    state,
    task: BoardTask | None,
) -> dict:
    """Return locally verifiable description-cap status without provider I/O.

    A configured provider that declares no positive cap remains distinguishable
    from a task with no active sync provider. Callers must not substitute a
    guessed limit in either case.
    """
    provider = _task_sync_validation_provider(state, task)
    if not provider:
        return {
...
def task_description_sync_constraint(
    ...
    status = task_description_sync_limit_status(state, task)
    return status if status["verifiable"] else None
...
class BoardSyncManager:
    ...
    def _task_sync_eligibility(
        ...
        *,
        ...
        auto_track = (
            task_auto_tracks_for_board_sync(self.state, task, settings)
        )
...
__all__ = [
    ...
    "task_auto_tracks_for_board_sync",
    "task_allows_auto_create_for_board_sync",
    "task_is_schedule_created",
    "task_is_tracked_for_board_sync",
]
