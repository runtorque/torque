...
- Public MCP tools use one canonical, role-neutral vocabulary projected by caller kind and Agent Class authority. Role-prefixed handler names are internal compatibility details and must not appear in prompts, skills, or operator guidance.
- Engineer scoping is strict: engineers see only themselves, owned workers/terminals, in-group tasks allowed by existing tool rules, and same-supervising-Architect Engineer peers for the explicit peer notify/inspect surface only. Peer notification grants read-only inspection of the referenced task/stream context; it does not grant generic peer Engineer/worker visibility or write/control access. Engineer-created workers/tasks are auto-stamped with ownership ids. Deleting an engineer clears those ids back to the user.
...
- Architect ↔ engineer messaging is the only architect cross-kind channel. Engineers may message only their hiring architect. Workers report only through the canonical task-status, derive, and ask tools.
