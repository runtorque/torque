...
- **Python package** (`loom/`):
  ...
  - `roles.py` — `RoleManager` (primary role discovery/save/delete surface; reads new role files from `~/.loom/roles` and project `.loom/roles`, compat-reads legacy templates from `~/.loom/agents` and project `.loom/agents`, renders worker preamble blocks from `preamble` + `priorities`, and keeps the stage-2 write path pointed at `roles/`)
...
- **Webview** (`webview.html` + `static/`):
  ...
  - `static/js/events.js`, `context.js`, `weaver.js` — live panels that rebuild from WebSocket updates (`weaver.js` now renders the Engineers panel). Preserve scroll anchors, focus/caret, inline drafts, expanded sections, and other in-progress operator context while updates stream in.
