...
The right-hand panel used to be called "Architects & Engineers" and was rendered by `renderEngineerPanel()` in `static/js/engineer.js:577`. It was built when there was exactly **one Engineer per group**, so it was scoped to the group and mixed two concerns:

1. **Roster surface** — lists every architect and engineer in the group, with inline "+ Add Architect" and "+ Add Engineer" buttons (see `static/js/engineer.js:629-630`) plus the architect decision-log controls.
2. **Engineer detail view** — Journal, Events, and Worklog tabs, all keyed by group (`state.engineer_worklog[group]`, `state.engineer_sent_events[group]`, `state.engineer_journal[group]`, `state.engineer_buffer_stats[group]`).
...
2. **Creation lives where agents live.** Architect / Engineer creation moves into the agents grid's `+ New` split button (`renderSplitBtn` in `static/js/render.js:72`), alongside the existing role/template entries and replacing the retired Engineer entry. The roster table disappears from the panel.
3. **No server-side churn in phase 1.** `state.engineer_worklog`, `state.engineer_sent_events`, `state.engineer_journal`, and `state.engineer_settings` remain keyed by group. The client filters at render time to the focused engineer's group. Re-keying by `agent_id` is tracked as a follow-up, not a blocker.
4. **Retire the Engineer UI surface.** The lingering "Engineer" menu entry (`_renderEngineerMenuItem` in `static/js/render.js:104`) and the `newEngineer` code path are removed in the same pass. The CLI `engineer_*` tool surface is already gone per CLAUDE.md invariants; this closes the last UI gap.
...
| worker    | **Events**, **Worklog**                          | Worklog = this worker's task history (assigned + completed). Events = this worker's own event stream (per-cell events, not engineer digest). |
...
For a worker, Worklog is its **task history**: every board task where `task.agent_id == worker.id`, newest first, showing title, lane, status, dispatched-at, and the action used. This re-uses the existing worklog row template in `_engineerRenderWorklogItem` (`static/js/engineer.js:1144`); only the data source changes.

### Worker Events semantics

For a worker, Events means its own activity stream — the per-cell events already surfaced in `static/js/events.js`. We reuse that renderer here rather than the Engineer digest queue renderer. Concretely: the "Events" tab for a worker delegates to the same function that powers the existing Events panel, filtered to this agent's cell ID.
...
**New file `static/js/agent_panel.js`.** Replaces `static/js/engineer.js` as the panel renderer. The engineer file is deleted in the same commit — we are not maintaining two parallel panels. Public surface:

- `renderAgentPanel()` — called from `render()` in `static/js/render.js` wherever `renderEngineerPanel()` is called today.
...
- Remove `_renderEngineerMenuItem` and its call site. Delete `newEngineer` and any helpers it drags in.
...
- Delete the `new_engineer` command handler if still wired up.
- Keep `engineer_*` state stores in place for now — they are still populated and still carry valid per-engineer data (the engineer *is* the engineer semantically). The follow-up plan (§"Follow-ups") re-keys them.

### Filtering strategy (phase 1)

For an engineer, the panel reads from the group-keyed stores (`state.engineer_worklog[agent.group]`, etc.) and filters to entries whose `agent_id` / `created_by_*` matches the focused engineer where a per-agent distinction exists. Where the store is inherently per-group (e.g. digest queue), we show the group's data but label it "for group *<name>*" so the user understands the scope.

For a worker, Worklog reads `state.board_tasks` directly, filtered by `agent_id == worker.id`. No dependency on the engineer stores.
...
For an architect, the Decisions section reads from the architect-specific decision store that the current panel already uses (`_engineerArchitectDecisionUi`, backed by delta ops). Filtering is by `architect_id`.

## Migration & rollout

**Phase 1 (this plan).**

1. Move `+ Add Architect` / `+ Add Engineer` into the `+ New` split-button menu. Retire the "Engineer" menu entry and `newEngineer` code path.
2. Replace `renderEngineerPanel` with `renderAgentPanel`, binding to `focusedItemId`.
...
4. Delete `static/js/engineer.js`. Keep the neutral `#panel-agent` root and `agent-panel-*` CSS prefixes.
...
- Re-key `engineer_worklog` / `engineer_sent_events` / `engineer_journal` / `engineer_buffer_stats` by `agent_id` server-side. At that point the engineer panel shows only its own data, not a group-scoped projection.
...
- Audit the `engineer_*` SQL tables and column names; rename to `engineer_*` to match terminology, with migration.
