...
| SQLite persistence and engineer journal state | None before this wave | `tests/test_db.py` covers round-trips for agents, tasks, schedules, panel events, engineer settings, and journal entries | Unit/integration | Schema migration edge cases and historical backfill paths remain thin |
...
| Event bus and health monitoring | Limited indirect coverage through engineer tests | `tests/test_events.py` covers session start handling, blocked-label clearing, and stuck-agent health checks | Unit/integration | Full broadcast throttling and panel-log persistence interactions remain thinner than core state updates |
...
| Native desktop shell lifecycle and launch guardrails | None before the desktop shell landed | `tests/test_desktop.py`, `tests/test_desktop_entrypoint.py`, and `tests/test_cli_desktop.py` cover desktop-only env setup, attach-vs-spawn safety, child shutdown ownership, missing-`pywebview` failure paths, and CLI launch behavior | Unit + focused launcher integration | Native-window smoke still depends on manual validation because browser automation does not prove `pywebview` behavior |
