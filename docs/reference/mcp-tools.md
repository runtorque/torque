...
| `task_amend` | Architect-only: append an immutable attributed correction using an atomic expected-`task_content_hash` comparison. Existing description bytes remain the exact prefix; the new hash invalidates active hash-pinned grants. A live executor receives only a hash/re-read advisory, never correction text in place of the durable record. |
