...
When enabled, creating an agent automatically creates a new git worktree branched from the directory's repository. Each agent gets its own branch (`agent-matrix/{id}-{name}`) and worktree path, so multiple agents can work on the same repo in parallel without conflicts. The worktree is cleaned up when the agent is removed.
