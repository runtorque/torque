...
Tasks in Torque move forward through lanes. They never bounce backward. This principle keeps the board readable at a glance --- you can look at the board and immediately understand where every piece of work stands without digging into history.
...
| Lane | Purpose |
|------|---------|
| **Backlog** | Tasks waiting to be started. New tasks land here by default. `torque_ask(...)` creates tasks here for human review. |
...
| **Done** | Completed tasks. Reached via `torque_done(...)` or cascade completion. |
...
When the implementing agent calls `torque_derive(description="Review auth implementation", action="review")`, the parent task stays in In Progress and its status changes to **On Review**. If `status` is omitted, the status defaults to the target action's name (e.g., deriving to `review` sets status "review").
...
When an agent derives a follow-up task via `torque_derive(...)`, two things happen:
...
When a task is marked done via `torque_done(...)`, Torque walks up the parent chain and completes any ancestor tasks whose children are all finished. This is **cascade completion** --- the mechanism that moves tasks from In Progress to Done when their pipeline chain resolves.

### How it works

1. The agent calls `torque_done(message="summary")`. The current task moves to **Done**.
2. Torque looks at the parent task (via `parent_task_id`).
...
Cascade only fires on `torque_done(...)` (and `torque_ready()`), never on derive. This prevents premature completion --- a review task that derives a fix task won't cascade the root task, because the fix task isn't done yet.

Some actions also opt into `auto_close_on_done`. That cleanup is evaluated at the **root-task lifecycle** level, not just when one derived task finishes. Torque only auto-closes an opted-in agent after the root task itself is **Done**, and it skips closure when unresolved descendants, queued same-agent follow-ups, or pending Engineer reply tasks still indicate expected follow-up work.
...
**Step 2** --- Agent A calls `torque_derive(description="Review auth implementation", action="review")`. The task's status changes to "On Review". T2 (review) appears as a subordinate card.
...
**Step 3** --- Agent B reviews and finds issues. Calls `torque_derive(description="Fix null check in auth.py", action="fix")`. T2 stays in **In Progress** as the parent of the new fix task, its status changes to "Fixing", and T3 (fix) appears as a new subordinate card. The root task's status also changes to "Fixing".
...
**Step 4** --- Agent C fixes the issue. Calls `torque_derive(description="Re-review after fix", action="review")`. T3 stays in **In Progress** as the parent of the new review task, its status changes to "On Review", and T4 (review) appears.
...
**Step 5** --- Agent D reviews and approves. Calls `torque_done(message="Approved, all tests pass")`. T4 moves to Done. **Cascade fires:**
...
When the designated engineer sends a worker a question with `engineer_agent_message(...)`, Torque creates a visible follow-up task for that side conversation. The worker answers with `torque_reply(...)`, and that follow-up task closes with an **answered** outcome.
