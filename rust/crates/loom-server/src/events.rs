...
async fn handle_event(
    ...
    for event in &events {
        let (agent, panel) = {
            ...
            let (agent, panel) = {
                let Some(cell) = st.agents.get_mut(&event.cell_id) else { continue; };
                ...
                let panel = match event.kind.as_str() {
                    "session_start" => Some(("agent_started".to_string(), "Session started".to_string(), false)),
                    "session_end" => Some((
                        "agent_finished".to_string(),
                        if event.summary.is_empty() { "Session ended".to_string() } else { event.summary.clone() },
                        false,
                    )),
                    "progress" | "activity_change" => Some(("agent_progress".to_string(), event.detail.clone(), true)),
                    "waiting" => Some(("agent_waiting".to_string(), event.detail.clone(), false)),
                    "error" => Some((
                        "agent_error".to_string(),
                        if agent.error_message.is_empty() { event.detail.clone() } else { agent.error_message.clone() },
                        false,
                    )),
                    _ => None,
                };
