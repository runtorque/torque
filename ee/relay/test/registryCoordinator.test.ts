...
test("StandaloneRegistryCoordinator enforces one active daemon owner and routes through it", async () => {
  ...
  const sentToDaemon = await coordinator.sendToDaemon("daemon-1", toDaemon);
  assert.equal(sentToDaemon.delivered, true);
  assert.equal(sentToDaemon.connectionId, "daemon-conn-1");
  assert.equal(sentToDaemon.epoch, 1);
  assert.equal(daemonOne.sent[0].id, "msg-client");
  assert.equal(await coordinator.isCurrentDaemonConnection("daemon-1", "daemon-conn-1", 1), true);
  ...
  const broadcast = await coordinator.broadcastToClients("daemon-1", fromDaemon);
  assert.equal(broadcast.delivered, 1);
  assert.deepEqual(broadcast.connectionIds, ["client-conn-1"]);
  ...
  assert.equal(await coordinator.isCurrentDaemonConnection("daemon-1", "daemon-conn-1", 1), false);
  assert.equal(await coordinator.isCurrentDaemonConnection("daemon-1", "daemon-conn-2", 2), true);
  ...
  const sentAfterReplace = await coordinator.sendToDaemon("daemon-1", afterReplace);
  assert.equal(sentAfterReplace.delivered, true);
  assert.equal(sentAfterReplace.connectionId, "daemon-conn-2");
  ...
  assert.equal(await coordinator.isCurrentDaemonConnection("daemon-1", "daemon-conn-2", 2), false);
});
