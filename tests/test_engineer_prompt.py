...
class EngineerPromptTests(unittest.TestCase):
    ...
    def test_prompt_includes_compact_operational_guidance_sections(self):
        ...
        self.assertNotIn("engineer_* MCP tools", prompt)
        self.assertIn("## Core orchestration model", prompt)
        self.assertIn("stream_list", prompt)
        self.assertIn("stream_get", prompt)
        self.assertIn("session_map", prompt)
        ...
        self.assertIn("inspect the action catalog with `action_list`", prompt)
        ...
        self.assertIn("consult `action_list`", prompt)
        self.assertIn("`action_get`", prompt)
        ...
        self.assertIn("boot_summary → journal_list", prompt)
        self.assertIn("summary is `empty`, `stale`, `refreshing`, or `error`", prompt)
        self.assertIn("session_map → event_list", prompt)
        self.assertIn("Keep\n   `session_map` as the deterministic snapshot", prompt)
        ...
        self.assertIn("post a non-blocking `user_note`", prompt)
    ...
    def test_prompt_includes_dispatch_decision_tree(self):
        ...
        self.assertIn("Default to `task_dispatch`", prompt)
        self.assertIn("N>1 ready independent tasks", prompt)
        self.assertIn("serial `task_dispatch` calls", prompt)
    ...
    def test_prompt_includes_structured_policy_overrides(self):
        ...
        self.assertIn("prefer `user_note` with a proposed wave", prompt)
    ...
    def test_engineer_prompt_includes_architect_escalation_guidance(self):
        ...
        self.assertIn("supervisor_message", prompt)
