...
test('architect context menu exposes dismiss and rehire controls', async () => {
  ...
  assert.match(menu.innerHTML, /New engineer/);
  assert.match(menu.innerHTML, /openAddEngineerForSection\(&quot;alpha&quot;, &quot;arch-a&quot;\)/);
  assert.match(menu.innerHTML, /Dismiss/);
  assert.doesNotMatch(menu.innerHTML, /Rehire/);
...
test('embedded terminal websocket URL carries the page client id', () => {
  const { context } = createEmbeddedTerminalHarness({
    _torqueClientId() { return 'client-test-123'; },
  });

  const url = runInContext(context, `_embeddedTerminalUrl({ id: 'agent/one' });`);

  assert.equal(
    url,
    'ws://localhost:9000/ws/terminal/agent%2Fone?client_id=client-test-123',
  );
});
...
test('client-scoped focus_update direct messages update local selection without delta seq', () => {
  const { context } = createWsRenderHarness();
  runInContext(context, `
    state.agents = {
      'agent-1': {
...
test('pending new-group materialization runs active-group state transition', () => {
  ...
  runInContext(context, `
    ...
    var _boardFocusedTask = '';
    var _boardAddingTask = true;
    var _boardAddingTaskDraft = 'alpha pending draft';
    var _boardAddingTaskAgent = '';
    var _boardAddingTaskLane = 'To Do';
    var _boardInlineDraftId = 'draft-alpha';
    var _boardInlineAttachments = [];
    ...
    var _boardQuickEditTask = '';
    var _boardQuickEditKind = '';
    var _boardCardsScrollTop = 33;
    var _boardActiveViewKey = 'alpha-view';
    selectedAgentId = 'agent-a';
    selectedTerminalId = 'agent-a';
    focusedItemId = 'agent-a';
    setActiveGroup('beta', { allowPending: true });
    var pendingState = {
      active: state.active_group,
      current: _currentGroup(),
      draft: _boardAddingTaskDraft,
    };
    _handleDelta({
      seq: 1,
      ops: [
        { op: 'group_update', name: 'beta', agents: ['agent-b'] },
      ],
    });
  `);

  assert.deepEqual(jsonValue(context, `pendingState`), {
    active: 'alpha',
    current: 'alpha',
    draft: 'alpha pending draft',
  });
  assert.equal(jsonValue(context, `state.active_group`), 'beta');
  assert.equal(jsonValue(context, `_currentGroup()`), 'beta');
  assert.equal(jsonValue(context, `_boardAddingTask`), false);
  assert.equal(jsonValue(context, `_boardAddingTaskDraft`), '');
  assert.equal(jsonValue(context, `_boardAddingTaskLane`), '');
  assert.equal(jsonValue(context, `selectedAgentId`), null);
  assert.equal(jsonValue(context, `focusedItemId`), null);
  assert.equal(
    jsonValue(context, `_activeGroupSurfaceStateByGroup.alpha.board.addingTaskDraft`),
    'alpha pending draft',
  );
  ...
  assert.equal(jsonValue(context, `_boardAddingTaskDraft`), 'alpha pending draft');
  assert.equal(jsonValue(context, `_boardAddingTaskLane`), 'To Do');
  assert.equal(jsonValue(context, `selectedAgentId`), 'agent-a');
  assert.equal(jsonValue(context, `focusedItemId`), 'agent-a');
});

test('server-driven active group deletion restores fallback group UI state', () => {
  const { context, sandbox } = createStandaloneWsSyncHarness();
  loadScript(context, 'static/js/modals/task-modal.js');
  ...
  runInContext(context, `
    ...
    state.standalone_panel_layout = {
      version: 1,
      bottom: { open: false, size: 240, tabs: [], active: '' },
      right: { open: true, size: 320, tabs: ['templates'], active: 'templates' },
      floats: {},
      last_active: 'templates',
    };
    _restoreStandalonePanelState();
    state.groups = { alpha: ['agent-a'], beta: ['agent-b'] };
    state.group_settings = {
      alpha: { collapsed_default: false },
      beta: { collapsed_default: false },
    };
    ...
    var _boardFocusedTask = '';
    var _boardAddingTask = true;
    var _boardAddingTaskDraft = 'alpha saved draft';
    var _boardAddingTaskAgent = '';
    var _boardAddingTaskLane = 'To Do';
    var _boardInlineDraftId = 'draft-alpha';
    var _boardInlineAttachments = [];
    ...
    focusedItemId = 'agent-b';
    _boardAddingTask = true;
    _boardAddingTaskDraft = 'beta deleted draft';
    _boardAddingTaskLane = 'Done';
    _boardCardsScrollTop = 9;
    _expectedSeq = 1;
    _handleDelta({
      seq: 1,
      ops: [
        { op: 'group_remove', name: 'beta' },
      ],
    });
  `);
  ...
  assert.equal(jsonValue(context, `state.active_group`), 'alpha');
  assert.equal(jsonValue(context, `_currentGroup()`), 'alpha');
  assert.equal(jsonValue(context, `_boardAddingTaskDraft`), 'alpha saved draft');
  assert.equal(jsonValue(context, `_boardAddingTaskLane`), 'To Do');
  assert.equal(jsonValue(context, `_boardCardsScrollTop`), 64);
  assert.equal(jsonValue(context, `selectedAgentId`), 'agent-a');
  assert.equal(jsonValue(context, `selectedTerminalId`), 'agent-a');
  assert.equal(jsonValue(context, `focusedItemId`), 'agent-a');
  assert.equal(
    jsonValue(context, `_activeGroupSurfaceStateByGroup.beta.board.addingTaskDraft`),
    'beta deleted draft',
  );
...
test('active group changes reload visible group-scoped Actions and Roles panels', () => {
  const { context, sandbox } = createMainHarness({
    renderTerminalWorkspace() {},
    tplEditorLoad() {
      if (!sandbox.actionLoadGroups) sandbox.actionLoadGroups = [];
      sandbox.actionLoadGroups.push(sandbox.state.active_group || '');
    },
    agentTemplateEditorLoad() {
...
test('Actions and Roles panels hide stale lists while loading the new active group', () => {
  ...
  loadScript(context, 'static/js/actions.js');
  loadScript(context, 'static/js/templates.js');
  const actionsEditor = document.register('tpled-editor');
  const rolesEditor = document.register('agent-tpl-editor');
  ...
  sandbox.state.children = {};
  sandbox.state.active_group = 'beta';
  ...
  runInContext(context, `
    ...
    _agentTplData = { name: 'AlphaRole', command: 'alpha' };
    renderAgentTemplatesPanel();
  `);
...
test('architect context menu New engineer opens a scoped modal and submits architect hire context', () => {
  const { context, document, sandbox } = createMainRenderHarness();
  ...
  const menuItem = context._cellContextMenuItems('arch-a')
    .find((item) => item && item.label === 'New engineer');
  assert.ok(menuItem, 'architect context menu should expose New engineer');
  assert.equal(menuItem.action, 'openAddEngineerForSection("torque", "arch-a")');
  vm.runInContext(menuItem.action, context);
...
test('grid-level New engineer submits a user-hired engineer without architect context', () => {
  ...
  assert.doesNotMatch(main.innerHTML, /data-agent-section="user"|data-agent-strata="engineers"/);
  ...
  const menuItem = context._agentGridNewMenuItems('torque')
    .find((item) => item && item.label === 'New engineer');
  assert.ok(menuItem, 'grid + New menu should expose standalone engineer creation');
  vm.runInContext(menuItem.action, context);
...
test('grid-level New worker affordance survives delta rerenders and uses standalone worker flow', () => {
  const { sandbox, document } = createSandbox({
  ...
  assert.doesNotMatch(main.innerHTML, /\+ Add Worker/);
  assert.ok(
  ...
  assert.doesNotMatch(main.innerHTML, /class="ghost-card ghost-card--engineer|class="ghost-card ghost-card--worker standalone-worker-card-new|\+ New Engineer|\+ Add Worker/);
  assert.doesNotMatch(main.innerHTML, /No orphan engineers\./);
  assert.doesNotMatch(main.innerHTML, /No orphan workers\./);
});

test('stratified grid renders Architects stratum and architect card without inline New Architect anchor', () => {
  const { context, document, sandbox } = createMainRenderHarness();
  const main = document.getElementById('main');
  ...
  assert.equal((main.innerHTML.match(/ghost-card ghost-card--architect/g) || []).length, 0);
...
test('grid-level New architect opens the group-scoped architect modal', () => {
  const { context, document, sandbox } = createMainRenderHarness();
  ...
  const menuItem = context._agentGridNewMenuItems('torque')
    .find((item) => item && item.label === 'New architect');
  assert.ok(menuItem, 'grid + New menu should expose standalone architect creation');
  vm.runInContext(menuItem.action, context);
  assert.equal(modalDom.modal.classList.contains('visible'), true);
...
test('main hierarchy drops stale focused creation controls across websocket delta rerenders', () => {
  const cases = [
    { label: 'worker ghost', focusId: 'grid-control:section-new-worker:torque:user', focusKey: 'section-new-worker:torque:user', principal: '' },
    { label: 'engineer ghost', focusId: 'grid-control:section-new-engineer:torque:architect:arch-a', focusKey: 'section-new-engineer:torque:architect:arch-a', principal: 'arch-a' },
    { label: 'architect ghost', focusId: 'grid-control:agent-new-architect:torque', focusKey: 'agent-new-architect:torque', principal: '' },
  ];
  ...
  loadScript(context, 'static/js/grid/main.js');
    ...
    runInContext(context, `
      focusedItemId = ${JSON.stringify(item.focusId)};
      _handleFullState({
        seq: 1,
        groups: { torque: ['arch-a'] },
        group_settings: { torque: { collapsed_default: false } },
    ...
    assert.notEqual(jsonValue(context, `focusedItemId`), item.focusId, item.label + ' should not remain focusable');
    assert.doesNotMatch(main.innerHTML, new RegExp('data-focus-key="' + item.focusKey.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '"'));
    assert.doesNotMatch(main.innerHTML, /ghost-card/);
    ...
    assert.notEqual(jsonValue(context, `focusedItemId`), item.focusId, item.label + ' should stay off removed controls');
    assert.doesNotMatch(main.innerHTML, new RegExp('data-focus-key="' + item.focusKey.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '"'));
    assert.doesNotMatch(main.innerHTML, /ghost-card/);
  }
});
...
test('grid-level + New dropdown exposes standalone creation flows', () => {
  const { context, document, sandbox } = createMainNavigationHarness();
  ...
  runInContext(context, `
    ...
    openAddWorkerForSection = function(group) {
      workerNewCalls.push({ group: group });
    };
  ...
  context.openAgentGridNewMenu({
    ...
    clientY: 2,
    currentTarget: {
      getBoundingClientRect() { return { left: 33, bottom: 44 }; },
    },
  }, 'torque');
...
test('main render restores the inline task description editor caret across rerenders', () => {
  ...
  const firstDetailHtml = runInContext(context, `renderAgentDetails(state.agents['agent-1'])`);
  const secondDetailHtml = runInContext(context, `renderAgentDetails(state.agents['agent-1'])`);
  assert.equal(secondDetailHtml, firstDetailHtml,
    'renderAgentDetails output stays byte-stable across an unchanged detail rerender');
  assert.match(firstDetailHtml, /Keep this draft steady across websocket refreshes/);
  ...
  input.value = 'Keep this draft steady across websocket refreshes after a websocket tick';
  input.selectionStart = 24;
  input.selectionEnd = 33;
  document.activeElement = input;

  runInContext(context, `
    state.agents['agent-1'].activity_detail = 'progress tick from websocket';
    render();
  `);

  assert.equal(input.focused, true);
  assert.equal(input.value, 'Keep this draft steady across websocket refreshes after a websocket tick');
  assert.equal(input.selectionStart, 24);
  assert.equal(input.selectionEnd, 33);
  assert.match(main.innerHTML, /Keep this draft steady across websocket refreshes after a websocket tick/);
  assert.equal(
    jsonValue(context, `_agentDetailState('agent-1').description_editor.draft`),
    'Keep this draft steady across websocket refreshes after a websocket tick',
  );
});
