...
class MCPScopingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_engineer_task_edit_rejects_over_limit_label_for_tracked_github_task(self):
        state = self._make_state()
        alice = self._add_engineer(state, "eng-alice", "Alice")
        state.update_group_settings(
            "torque",
        ...
        async def fake_handle_command(_payload):
            self.fail("over-limit label should be rejected before mutation")
        ...
        self.assertTrue(is_error)
        self.assertIn("Label name is 51 characters", text)
        self.assertEqual(task.labels, ["normal"])
